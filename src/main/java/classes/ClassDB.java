...
public class ClassDB {
    ...
    public DeliveryPoint getProductByAddress(String address) throws SQLException, ClassNotFoundException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        Connection connect = this.connect();
        PreparedStatement statement = connect.prepareStatement("SELECT * FROM order_products where address=? ; ");
    ...
    public List<DeliveryPoint> getCustomersProduct(Customer customer) throws SQLException, ClassNotFoundException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        Connection connect = this.connect();
        PreparedStatement statement = connect.prepareStatement("SELECT * FROM point_and_customer JOIN order_products on point_and_customer.address = order_products.address JOIN product on order_products.product_name = product.product_name where customer_id=?");
    ...
    public List<Product> getCustomersProductListProduct(Customer customer) throws SQLException, ClassNotFoundException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        int customerId = customer.getId();
        Connection connect = this.connect();
        PreparedStatement statement = connect.prepareStatement("SELECT * FROM order_products,  delivery_point.point_and_customer \n" +
        ...
        while (resultSet.next()) {
            ...
            products.add(new Product(name, number, price, date));
        }
        connect.close();
        return products;
    }
